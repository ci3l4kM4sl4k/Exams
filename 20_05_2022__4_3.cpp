...
int main()
{
    string fileName = "numbers.txt", fileName1 = "triples.txt"; //names of files
    fstream file, file1;
    int arr[200], amountThrees = 0, amountFives = 0;

    file.open(fileName, ios::in); //imports all numbers from the first file
        for(int i = 0; i < 200; i++)
            file >> arr[i];
    file.close();
    ...
    for(int i = 0; i < 200; i++)
        for(int j = 0; j < 200; j++)
            for(int z = 0; z < 200; z++)
                ...
                {
                    file1.open(fileName1, ios::app); //exports all triples of numbers to secound file
